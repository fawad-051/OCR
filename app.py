...
# ---------- OS-Specific Tesseract Configuration ----------
...
def detect_table_and_extract(image: Image.Image, lang='eng'):
    """Try to detect table structure and extract as DataFrame. If no table found, return None."""
    ...
    for r in range(nrows):
        ...
        for c in range(ncols):
            x1, x2 = xs_c[c], xs_c[c+1]
            y1, y2 = ys_c[r], ys_c[r+1]
            ...
            if cell.size == 0:
                ...
                text = pytesseract.image_to_string(pil_cell, lang=lang).replace('\n', ' ').strip()
...
def image_to_docx(images, output_docx_path, lang='eng'):
    """Process images: extract tables or text, write to DOCX."""
    doc = Document()
...
def main():
    ...
    uploaded_file = st.file_uploader(
        ...
        help="For mobile users: prefer files <50MB and stable Wi-Fi connection."
    )

    lang = st.text_input('Tesseract language (e.g. eng, eng+urd)', value='eng')
    if not lang:
        lang = 'eng'
    ...
    st.markdown("- Install Tesseract OCR and Poppler (system level).")
    st.markdown("- Python packages: `streamlit`, `pillow`, `pytesseract`, `pdf2image`, `opencv-python-headless`, `python-docx`, `pandas`, `numpy`.")
    st.markdown("- Add `tesseract-ocr` and `poppler-utils` to your `packages.txt` for Streamlit Cloud.")
